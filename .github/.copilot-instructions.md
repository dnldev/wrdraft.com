...
trust these instructions as the primary source of truth.**
...
application's main features are an intelligent "Matchup Calculator" and a "Match History" system for personalized
performance tracking.

### Repository Information

- **Project Type:** Web Application (Multi-Page App with Static Generation)
...
2. **Database Seeding:**
   ...
   - **Purpose:** Seeds or updates the static game data in the Upstash database. This is a **non-destructive**
     operation.
...
- `app/`: Next.js App Router. Each top-level directory (e.g., `/`, `/team-comps`, `/history`) represents a distinct
  page.
- `lib/upstash.ts`: **Centralized Upstash Redis client instance**. It uses a lazy-initialization pattern.
- `lib/data-fetching.ts`: Contains the `getPlaybookData` function for server-side data fetching.
...
1. **Routing:** The application uses a multi-page architecture based on the App Router's file system. Navigation is
   handled by links and `router.push`, not query parameters.
2. **Database Interaction:** All server-side database operations **must** use the official `@upstash/redis` SDK via the
   `getKvClient()` function in `lib/upstash.ts`.
3. **Data Flow:** Each `page.tsx` file is a Server Component responsible for its own data fetching via
   `getPlaybookData()`. There is no data passed down from the root layout.
4. **Static Generation:**
   - Pages that display non-user-specific data (e.g., `/`, `/team-comps`, `/pairings`) **must** be statically
     generated (the default behavior).
   - Pages that display user-specific or frequently changing data (e.g., `/history`) **must** be dynamically rendered
     by exporting `export const dynamic = "force-dynamic";`.
5. Look out for LLM looking comments and point out to remove them
